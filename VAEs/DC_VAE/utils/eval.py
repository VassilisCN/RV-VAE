...
def compute_fid_recon(netE, netD, dl, zdim, eval_bs=128, buf_dir=None, rv=False):
    if buf_dir is None:
    ...
    while total <= num_eval_imgs:
        for iter_idx, (imgs, _) in enumerate(tqdm(dl)):
            ...
            rec, _, _ = f_recon(imgs, netE, netD, zdim, mode="train", clipping=False, rv=rv)
            if rv:
                rec = rec[:,0,...]
...
def compute_fid_sample(netD, zdim, eval_bs=128, buf_dir=None, rv=False):
    if buf_dir is None:
        buf_dir = f"tmp/{time.time()}"
    if not os.path.exists(buf_dir):
        os.makedirs(buf_dir)
    ...
    while total <= num_eval_imgs:
        ...
        if rv:
            z_var = torch.cuda.FloatTensor(np.zeros(shape=(eval_bs, zdim)))
            curr_z = torch.unsqueeze(curr_z, 1)
            z_var = torch.unsqueeze(z_var, 1)
            curr_z = torch.cat((curr_z, z_var), 1)
        imgs = netD(curr_z)
        if rv:
            imgs = imgs[:,0,...]
